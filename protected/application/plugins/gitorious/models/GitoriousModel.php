...
*
*/
class GitoriousModel extends SourceModel {

	protected $_name 	= 'gitorious_data';

	protected $_prefix = 'gitorious';

	protected $_search  = 'content';

	protected $_update_tweet = "Did %d things at gitorious.org on my lifestream %s";
	...
	public function updateData() {
		...
		if ($http_code != 200) {
			throw new Stuffpress_Exception( "Gitorious returned http status $http_code for url: $url", $http_code );
		}

		if (!($items = simplexml_load_string($response))) {
			throw new Stuffpress_Exception( "Gitorious did not return any result", 0 );
		}
