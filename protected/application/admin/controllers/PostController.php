...
class Admin_PostController extends Admin_BaseController
{
	...
	public function deleteAction() {
		...
		// Can we do a "real" delete?
		$full_delete = ( $source_id == $this->_properties->getProperty('stuffpress_source') );
		...
		if ( $full_delete ) {
			...
			// This is from an external source, we have to permanently hide it
			$data->unwantItem( $source_id, $item_id );
		}
	...
	public function unwantedAction() {
		...
		$data->unwantedItem( $id );
		$source->unwantedItem( $id );
// 		// All checks ok, we can delete !
// 		$data->deleteItem($source_id, $item_id);
// 		$source->deleteItem($item_id);
//
